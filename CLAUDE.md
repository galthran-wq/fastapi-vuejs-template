...
Tests run against a separate `${POSTGRES_DB}_test` database. The test commands set `POSTGRES_DB` to the test DB and execute inside the server container.
...
1. Copy env files: `cp .env.example .env && cp server/.env.example server/.env`
